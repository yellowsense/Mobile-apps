...
const Signin = () => {
  ...
  const RegisterSchema = Yup.object().shape({
    ...
    // .required('Please enter your email adddress.'),
  ...
  const handleRegister = async values => {
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        if (data.message === 'User registration successful') {
          ...
          Toast.show({
            type: 'success',
            position: 'top',
            text1: 'Regsiter Successfully',
            text2: `Hi ${values.name}, You have Regsiter successfully!`,
            visibilityTime: 2000,
            autoHide: true,
            onShow: () => {
          ...
          Alert.alert(
            'Error',
            'Unexpected register response. Please try again.',
          );
        }
      } else {
        Alert.alert(
      ...
      Toast.show({
        type: 'error',
        position: 'top',
        text1: 'Regsiteration Failed!',
        text2: 'User already registered. Please login.',
        visibilityTime: 3000,
        autoHide: true,
        onShow: () => {
