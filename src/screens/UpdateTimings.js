...
const UpdateTimings = ({route}) => {
  ...
  const handleUpdateTimings = async () => {
    try {
      ...
      Toast.show({
        type: 'success',
        text1: 'Timings updated successfully',
        position: 'top',
        visibilityTime: 2000,
        autoHide: true,
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        <ScreenHeader title="Update Availability" />
        ...
        {slots.map((slot, index) => (
          <View key={index} style={styles.personalInfo}>
            ...
            <View
              ...
              <Text style={{fontSize: 16,color:"gray"}}>{slot.fromTime}</Text>
              <Text style={{fontSize: 16,color:"gray"}}>{slot.toTime}</Text>
            </View>
